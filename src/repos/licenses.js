...
async function bindMachineIfEmpty(licenseId, machineId){
  return tx(async (client)=>{
    const cur = await client.query(
      `SELECT id, bound_machine_id FROM licenses WHERE id=$1 FOR UPDATE`,
      [licenseId]
    );
    ...
    if(!bound){
      await client.query(
        `UPDATE licenses SET bound_machine_id=$2, activated_at=NOW() WHERE id=$1`,
        [licenseId, machineId]
      );
    }
...
async function countUsers(licenseId){
  const r = await query(
    `SELECT COUNT(*)::int AS c FROM users WHERE license_id=$1`,
    [licenseId]
  );
  return r.rows[0]?.c ?? 0;
}
